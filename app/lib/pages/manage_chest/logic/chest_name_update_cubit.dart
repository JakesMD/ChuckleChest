...
class CChestNameUpdateState
  ...
  ///
  /// The initial state.
  CChestNameUpdateState.initial() : name = '', super.initial();
...
class CChestNameUpdateCubit extends Cubit<CChestNameUpdateState> {
  ...
  CChestNameUpdateCubit({required this.chestRepository, required this.chestID})
  ...
  Future<void> updateChestName({required String name}) async {
    ...
    final result = await chestRepository
        .updateChest(chestID: chestID, name: name)
        .run();
